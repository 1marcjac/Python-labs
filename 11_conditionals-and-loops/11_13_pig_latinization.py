...
def translate_to_pig_latin(word):
    ...
    while len(word) > 0 and not word[-1].isalpha():
        suffix_punctuation = word[-1] + suffix_punctuation
        word = word[:-1]
    
    # If there's no alphabetic characters left, return the original "word" (punctuation)
    if len(word) == 0:
        return prefix_punctuation + suffix_punctuation
    ...
    # Return the word with original punctuation
    return prefix_punctuation + pig_latin + suffix_punctuation
...
I decide then that I need to rest and eat something. Thats when I opened my backpack and saw...
...
translated_words =[translate_to_pig_latin(word) for word in text ]

pig_latin_text = ''.join(translated_words)
